...
{
    public partial class Expire_Soon_medi : Form
    {
        SqlConnection con = new SqlConnection("Data Source=DESKTOP-0RQOAPG;Initial Catalog=pharmacy;Integrated Security=True");
        public Expire_Soon_medi()
        {
            InitializeComponent();
        }

        private void Expire_Soon_medi_Load(object sender, EventArgs e)
        {
            SqlCommand cmd = new SqlCommand("SELECT * FROM medicine WHERE DATEDIFF(Month,GETDATE(),Exdate)=6;", con);
